...
tamanhos_cabecalho = {'Preço Unitário': 5, 'Mão de Obra': 5, 'Composições Analíticas': 3,
                      'ABC de Serviços': 4, 'Resumido': 4, 'Orçamento Sintético': 4, 'Orçamento Analítico': 3, 'ABC de Insumos': 5}
colunas_titulo = {'Preço Unitário': 'C', 'Mão de Obra': 'D', 'Composições Analíticas': 'C', 'ABC de Serviços': 'C',
                  'Resumido': 'D', 'Orçamento Sintético': 'D', 'Orçamento Analítico': 'C', 'ABC de Insumos': 'C'}
banco_dados = {'Preço Unitário': 'E1:F2', 'Mão de Obra': 'E1:G2', 'Composições Analíticas': 'E1:G2', 'ABC de Serviços': 'D1:D2',
               'Resumido': 'E1:E2', 'Orçamento Sintético': 'E1:F2', 'Orçamento Analítico': 'E1:F2', 'ABC de Insumos': 'D1:D2'}
nomes_relatorios = {'Preço Unitário': {'ordem': '09 - ', 'relatorio': ' - Composições com Preço Unitário'}, 'Mão de Obra': {'ordem': '08 - ', 'relatorio': ' - Sintético com Valor da Mão de Obra, Equipamento e Material'}, 'Composições Analíticas': {'ordem': '07 - ', 'relatorio': ' - Composições Analíticas com Detalhamento de LS e BDI'}, 'ABC de Serviços': {
  'ordem': '06 - ', 'relatorio': ' - Curva ABC de Serviços'}, 'ABC de Insumos': {'ordem': '05 - ', 'relatorio': ' - Curva ABC de Insumos'}, 'Orçamento Analítico': {'ordem': '04 - ', 'relatorio': ' - Orçamento Analítico'}, 'Resumido': {'ordem': '02 - ', 'relatorio': ' - Orçamento Resumido'}, 'Orçamento Sintético': {'ordem': '03 - ', 'relatorio': ' - Orçamento Sintético'}}
      ...
      if cel.row > tamanho_cabecalho - 1:
        if str(cel.value).find(".") != -1 and isinstance(cel.value, (int, float)) and len(str(cel.value)) > 4:
          if (len(str(cel.value).split(".")[1]) > 2):
            length = len(str(cel.value).split(
              ".")[0] + "." + str(cel.value).split(".")[1][0:1])
          else:
...
def remover_nome_bloqueado(file, aba):
  ...
  aba[f'{coluna_titulo}2'].value = aba[f'{
    coluna_titulo}2'].value.replace(' - "BLOQUEADO"', '')
  aba[f'{coluna_titulo}2'].value = aba[f'{
    coluna_titulo}2'].value.replace(' - "Bloqueado"', '')
  aba[f'{coluna_titulo}2'].value = aba[f'{
    coluna_titulo}2'].value.replace(' - "bloqueado"', '')
